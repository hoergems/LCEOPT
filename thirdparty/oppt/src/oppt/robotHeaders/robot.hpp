...
public:
     ...
     *
     */
    PropagationResultSharedPtr propagateState(PropagationRequestSharedPtr& propagationRequest);
     ...
     */
    ObservationResultSharedPtr makeObservationReport(ObservationRequestSharedPtr& observationRequest) const;
     ...
     */
    CollisionReportSharedPtr makeDiscreteCollisionReportDirty() const;
     ...
     */
    oppt::CollisionReportSharedPtr makeDiscreteCollisionReport(const oppt::RobotStateSharedPtr& state) const;
    ...
    void updateViewer(const oppt::RobotStateSharedPtr& state,
     ...
     */
    VectorString getCollisionCheckedLinks() const;
...
private:
    ...
    VectorGeometryUniquePtr visualRobotGeometries_;

private:
    bool makeActionSpace(ActionSpaceInfo& actionSpaceInfo);

    bool makeStateSpace(StateSpaceInfo& stateSpaceInfo);

    bool makeObservationSpace(ObservationSpaceInfo& observationSpaceInfo);
    ...
    void setupViewer(const std::string &robotName, const std::string &environmentFile, const unsigned int &frameRate);

    void addBodyCallback(Body *const body);

    void removeBodyCallback(std::string name);

    void changeBodyPoseCallback(const std::string& name, const geometric::Pose& poseVec);

    void createRobotCollisionObjects_(const oppt::RobotStateSharedPtr& state) const;

    bool init();

    void setEnvironmentInfo(oppt::EnvironmentInfoSharedPtr& environmentInfo);

    void initEnvironmentCallbacks();
    ...
    void setCollisionsAllowed(const bool& collisionsAllowed) const;
