...
public:
    ...
    virtual bool insideStateLimits(const oppt::RobotState& state) const = 0;
...
public:
    ...
    virtual bool insideStateLimits(const oppt::RobotState& state) const override {
        const VectorFloat *stateVec = &(static_cast<const oppt::VectorState&>(state).state_);
        for (size_t i = 0; i < (*stateVec).size(); i++) {
            if ((*stateVec)[i] < lowerLimits_[i]) {
                return false;
            } else if ((*stateVec)[i] > upperLimits_[i]) {
...
public:
    ...
    bool enforceStateLimits(oppt::RobotState &state) const {
        if (!stateLimits_) {
            WARNING("Trying to enforce state limits, but no limits set");
            return false;
     ...
     */
    bool insideStateLimits(const oppt::RobotState& state) const {
