        ...
        const sdf::ElementPtr& linkElement) const
{
    ...
    geometric::Pose linkWorldPose(processPoseElement(linkPoseElement) +
                                  processPoseElement(modelPoseElement));
    BodyUniquePtr body(new Body(modelName, linkWorldPose));
    body->scopedName_ = modelName + "::" + linkElement->Get<std::string>("name");
    while (collisionElement) {
        std::string collisionName = collisionElement->Get<std::string>("name");
        if (collisionName != "__default__") {
            GeometryUniquePtr collisionGeometry = std::move(makeCollisionGeometry(collisionElement, linkWorldPose));
    ...
    while (visualElement) {
        ...
        if (visualName != "__default__") {
            GeometryUniquePtr visualGeometry = std::move(makeVisualGeometry(visualElement, linkWorldPose));
            if (!visualGeometry)
...
geometric::Pose SDFEnvironmentParser::processPoseElement(const sdf::ElementPtr& poseElement) const
{
    ...
    if (poseElement) {
        if (poseElement->GetValue()) {
            ...
            pose = geometric::Pose(poseVec[0], poseVec[1], poseVec[2], poseVec[3], poseVec[4], poseVec[5]);
            return pose;
        } else {
...
geometric::Pose SDFEnvironmentParser::getCollisionPose(const sdf::ElementPtr &collisionElement, const geometric::Pose &parentLinkPose) const {
    ...
    geometric::Pose collisionPose = geometric::Pose(collisionPoseLocal + parentLinkPose);
