...
std::string Body::toSDFString() const {
    ...
    for (auto &collisionObject : opptCollisionObjects_) {
        ...
        auto pose = (collisionObject->getCollisionGeometry()->getWorldPose() - worldPose_);
        serString += "<pose frame=''>";
        serString += std::to_string(pose.position.x()) + " ";
        serString += std::to_string(pose.position.y()) + " ";
        serString += std::to_string(pose.position.z()) + " ";
        auto eulerAngles = oppt::math::quaternionToEulerAngles(pose.orientation);
        serString += std::to_string(eulerAngles.x()) + " ";
        serString += std::to_string(eulerAngles.y()) + " ";
        serString += std::to_string(eulerAngles.z());
        serString += "</pose>";
    ...
    for (auto &visualGeometry : visualGeometries_) {
        ...
        auto pose = (visualGeometry->getWorldPose() - worldPose_);
        serString += "<pose frame=''>";
        serString += std::to_string(pose.position.x()) + " ";
        serString += std::to_string(pose.position.y()) + " ";
        serString += std::to_string(pose.position.z()) + " ";
