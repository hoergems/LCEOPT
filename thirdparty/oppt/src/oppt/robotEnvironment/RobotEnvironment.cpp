...
void RobotEnvironment::onAddBody(const std::string& str)
{
    ...
    if (prefix_ == "exec") {
        EnvironmentChangeSharedPtr change = std::make_shared<BodyAddedChangeSDF>(str);
...
void RobotEnvironment::applyChanges()
{
    ...
    while (change) {
        ...
        case EnvironmentChangeType::BODY_ADDED_SDF: {
            //LOGGING("Apply body added change");
            std::string sdfString = change->as<BodyAddedChangeSDF>()->getSDFString();
        ...
        case EnvironmentChangeType::BODY_ADDED: {
            BodyUniquePtr body = change->as<BodyAddedChange>()->getBody()->clone();
            if (body)
                addBody(std::move(body), change->requiresCallback());
            notifyPlanningEnvironments(change);
            break;
        }
...
void RobotEnvironment::onEnvironmentChanged(const EnvironmentChangeSharedPtr& environmentChange)
{
    ...
    case EnvironmentChangeType::BODY_ADDED_SDF: {
        std::string sdfString = environmentChange->as<BodyAddedChangeSDF>()->getSDFString();
        SDFEnvironmentParser sdfParser;
        VectorBodyUniquePtr bodies = sdfParser.parseBodiesFromSDFString(sdfString);
        for (size_t i = 0; i != bodies.size(); ++i) {
            if (bodies[i]) {
    ...
    case EnvironmentChangeType::BODY_ADDED: {
        BodyUniquePtr body = environmentChange->as<BodyAddedChange>()->getBody()->clone();
        if (body) {
            addBody(std::move(body), false);
        }
...
bool RobotEnvironment::createRobot(const std::string &worldFile,
    ...
    robot_ = std::make_unique<Robot>(worldFile_,
                                     robotName,
                                     configFile,
                                     prefix_,
                                     threadId);
