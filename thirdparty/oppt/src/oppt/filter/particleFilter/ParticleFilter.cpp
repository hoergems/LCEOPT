...
FilterResultPtr ParticleFilter::propagateParticles(const FilterRequestPtr& filterRequest)
{
    ...
    auto randomEngine = filterRequest->robotEnvironment->getRobot()->getRandomEngine();
    if (deficit > 0) {
        auto sampledParticles = particleSet.sampleWeighted(randomEngine, deficit);
        for (size_t i = particleCounter; i != deficit; ++i) {
            ...
            if (filterRequest->robotEnvironment->isTerminal(propagationRes) == false || filterRequest->allowTerminalStates) {
