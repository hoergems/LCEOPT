...
#ifdef GZ_SUPPORT
RobotSerializer::RobotSerializer(const std::string& worldFile):
    VectorSerializer(),
    worldFile_(worldFile),
    world_(nullptr) { }

void RobotSerializer::setWorld(const WorldPtr& world) {
    world_ = world;
}
#else
RobotSerializer::RobotSerializer(const std::string& worldFile):
...
void RobotSerializer::setStateSpace(StateSpace* stateSpace)
{
    stateSpace_ = stateSpace;
}

RobotStateSharedPtr RobotSerializer::loadState(const std::string& input) const
{
...
OpptUserDataSharedPtr RobotSerializer::loadUserData(std::istream& is) const {
