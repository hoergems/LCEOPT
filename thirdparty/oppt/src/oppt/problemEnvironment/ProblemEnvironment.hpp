...
public:
    ...
    int setup(int argc, char const* argv[]) {
        try {            
            ...
            robotExecutionEnvironment_ =
                makeRobotEnvironment<ProblemEnvironmentOptions>(problemEnvironmentOptions_.get(),
                        "exec");
            robotExecutionEnvironment_->setExecutionEnvironment(true);
            robotPlanningEnvironment_ = makeRobotEnvironment<ProblemEnvironmentOptions>(problemEnvironmentOptions_.get(),
                                        "planning");
            robotExecutionEnvironment_->setRandomEngine(randGen_);
            robotPlanningEnvironment_->setRandomEngine(randGen_);           
            ...
            onStepFinishedInternalFn_ = [this, resultsDir](const unsigned int & run, const unsigned int & step) {
            ...
            onRunFinishedFn_ = [this](const unsigned int & run) {
                ...
                resetRobotEnvironment<ProblemEnvironmentOptions>(robotExecutionEnvironment_.get(),
                        problemEnvironmentOptions_.get());
                resetRobotEnvironment<ProblemEnvironmentOptions>(robotPlanningEnvironment_.get(),
                        problemEnvironmentOptions_.get());
            };
    ...
    std::string getLogFilePath(const std::string & resultsDir) const {
        unsigned int i = 0;
        std::string envName = "";
        std::string filename =
    ...
    void setRunFinishedFn(std::function<void(const unsigned int&)> onRunFinishedFn) {
        onRunFinishedFn_ = onRunFinishedFn;
    }

    void setRunStartingFn(std::function<void(const unsigned int&)> runStartingFn) {
        runStartingFn_ = runStartingFn;
    }
...
protected:
    ...
    std::stringstream printOs;

private:
    std::function<void(const unsigned int&, const unsigned int&)> onStepFinishedInternalFn_ = nullptr;

    std::function<void(const unsigned int&)> onRunFinishedFn_ = nullptr;

    std::function<void(const unsigned int&)> runStartingFn_ = nullptr;
...
private:
    ...
    std::unique_ptr<oppt::RobotEnvironment> makeRobotEnvironment(OptionsType * options,
        ...
        if (prefix == "planning") {
            created = robotEnvironment->createRobot(options->planningEnvironmentPath,
                                                    options->robotName,
                                                    options->configPath,
                                                    prefix,
                                                    threadId);
            ...
            created = robotEnvironment->createRobot(options->executionEnvironmentPath,
                                                    options->robotName,
                                                    options->configPath,
                                                    prefix);
            robotName = robotEnvironment->getRobot()->getName();
    ...
    void resetRobotEnvironment(oppt::RobotEnvironment * robotEnvironment,
        ...
        for (auto & env : robotEnvironment->getRegisteredRobotEnvironments()) {
            resetRobotEnvironment<OptionsType>(env, options);
        }
    }
    ...
    SimulationResult run(const unsigned int& run, std::ofstream & os, int argc, char const * argv[]) {
        ...
        simulationResult.initialState = currentState;
        ...
        while (true) {
            ...
            std::stringstream ss;
            ss << "action: " << *(robotPlanningEnvironment_->getRobot()->getActionSpace()->denormalizeAction(action)) << "\n";
            PRINT(ss.str());
            //PRINT("action")
        ...
        if (finalState) {
            ...
            simulationResult.finalState = finalState;
        }
        os << "\n";
        os << "FINAL_STATE_END\n";
