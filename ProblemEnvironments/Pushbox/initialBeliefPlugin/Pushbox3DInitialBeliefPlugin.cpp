...
class Pushbox3DInitialBeliefPlugin: public InitialBeliefPlugin
{
public:
    Pushbox3DInitialBeliefPlugin():
        InitialBeliefPlugin() {
    }

    virtual ~Pushbox3DInitialBeliefPlugin() = default;
    ...
    virtual RobotStateSharedPtr sampleAnInitState() override {
        ...
        initRobotPosition.z() = initialStateVec[2];

        initBoxPosition.x() = initialStateVec[3] + truncatedNormal_->sample((*(robotEnvironment_->getRobot()->getRandomEngine().get())));
        initBoxPosition.y() = initialStateVec[4] + truncatedNormal_->sample((*(robotEnvironment_->getRobot()->getRandomEngine().get())));
        initBoxPosition.z() = initialStateVec[5];// + truncatedNormal_->sample((*(robotEnvironment_->getRobot()->getRandomEngine().get())));
        

        RobotStateSharedPtr initialState(new Pushbox3DState(initRobotPosition, initBoxPosition));
...
OPPT_REGISTER_INITIAL_BELIEF_PLUGIN(Pushbox3DInitialBeliefPlugin)
