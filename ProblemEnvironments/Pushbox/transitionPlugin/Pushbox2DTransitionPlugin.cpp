...
public :
    ...
    virtual PropagationResultSharedPtr propagateState(const PropagationRequest* propagationRequest) const override {
        ...
        auto state = propagationRequest->currentState->as<Pushbox2DState>();
        ...
        if (speedSquared > 0) {
            ...
            Vector2f closest(robotPosition.x() + delta[0]*t, robotPosition.y() + delta[1]*t);
            auto bpc = boxPosition - closest;
            //FloatType lineToBoxDistanceSquared = (boxPosition - closest).squaredNorm();

            FloatType lineToBoxDistanceSquared = std::pow(boxPosition.x() - closest.x(), 2) + std::pow(boxPosition.y() - closest.y(), 2);
        ...
        Vector2f nextRobotPosition(robotPosition.x() + delta[0], robotPosition.y() + delta[1]);

        propRes->nextState = RobotStateSharedPtr(new Pushbox2DState(nextRobotPosition, nextBoxPosition));
        ...
        if (updateGazebo and (robotEnvironment_->isExecutionEnvironment() or
            ...
            geometric::Pose robotPose(nextRobotPosition.x(), nextRobotPosition.y(), 0.0, 0.0, 0.0, 0.0);
            geometric::Pose opponentPose(nextBoxPosition.x(), nextBoxPosition.y(), 0.0, 0.0, 0.0, 0.0);
